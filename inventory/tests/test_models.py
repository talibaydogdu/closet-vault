...
def test_first_photo_is_cover_and_only_one(entities, image_upload):
    obj = item(entities)
    first = Photo.objects.create(item=obj, image=image_upload())
    second = Photo.objects.create(item=obj, image=image_upload(name="b.jpg"))
    assert first.is_cover and not second.is_cover
    second.set_as_cover()
    first.refresh_from_db()
    assert second.is_cover and not first.is_cover
...
def test_storage_relation_and_tags(entities):
    obj = item(entities)
    tag = Tag.objects.create(name="Favori")
    obj.tags.add(tag)
