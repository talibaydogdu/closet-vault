...
def make_item(entities):
    p,c,u=entities; return Item.objects.create(name="X",person=p,category=c,storage_unit=u,primary_color="Mavi")
def test_resize_ratio_thumbnail_and_webp(entities, image_upload):
    ...
    with Image.open(photo.image) as im: assert im.size == (1600,800) and im.format == "WEBP"
    with Image.open(photo.thumbnail) as thumb: assert max(thumb.size) == 400
    assert photo.image.name.endswith(".webp") and "photo.jpg" not in photo.image.name
def test_exif_orientation(entities):
    exif=Image.Exif(); exif[274]=6; data=BytesIO(); Image.new("RGB",(1200,600)).save(data,"JPEG",exif=exif)
    ...
    with Image.open(photo.image) as im: assert im.height > im.width
def test_bad_file_rejected_without_record(entities):
    ...
    with pytest.raises(ValidationError): Photo.objects.create(item=make_item(entities),image=upload)
