...
class Person(TimestampedModel):
    ...
    class Meta: ordering = ["name"]; verbose_name = "Kişi"; verbose_name_plural = "Kişiler"
    def __str__(self): return self.name


class Category(models.Model):
    name = models.CharField("Ad", max_length=100)
    parent = models.ForeignKey("self", verbose_name="Üst kategori", blank=True, null=True, on_delete=models.PROTECT, related_name="children")
    is_active = models.BooleanField("Aktif", default=True)
    sort_order = models.PositiveIntegerField("Sıra", default=0)
    class Meta:
        ordering = ["sort_order", "name"]
        constraints = [models.UniqueConstraint(fields=["parent", "name"], name="unique_category_parent_name")]
...
class StorageUnit(TimestampedModel):
    ...
    is_active = models.BooleanField("Aktif", default=True)
    class Meta: ordering = ["name"]; indexes = [models.Index(fields=["unit_type", "is_active"])]
    def __str__(self): return self.name
...
class ItemQuerySet(models.QuerySet):
    def visible(self): return self.exclude(status=Item.Status.ARCHIVED).filter(archived_at__isnull=True)
...
class Item(TimestampedModel):
    ...
    person = models.ForeignKey(Person, verbose_name="Kişi", on_delete=models.PROTECT, related_name="items")
    category = models.ForeignKey(Category, verbose_name="Kategori", on_delete=models.PROTECT, related_name="items")
    storage_unit = models.ForeignKey(StorageUnit, verbose_name="Saklama birimi", on_delete=models.PROTECT, related_name="items")
    ...
    class Meta:
        ordering = ["-created_at"]
        indexes = [models.Index(fields=["status", "archived_at"]), models.Index(fields=["person", "category"]), models.Index(fields=["primary_color"]), models.Index(fields=["brand"]), models.Index(fields=["storage_unit"])]
    @property
    def physical_location(self): return self.storage_unit.location_text
    @property
    def cover_photo(self): return next((p for p in self.photos.all() if p.is_cover), None)
    def archive(self):
...
def photo_path(instance, filename): return f"items/{instance.item_id}/{filename}"
def thumbnail_path(instance, filename): return f"items/{instance.item_id}/thumbs/{filename}"


class Photo(models.Model):
    item = models.ForeignKey(Item, verbose_name="Ürün", on_delete=models.CASCADE, related_name="photos")
    ...
    class Meta:
        ...
        constraints = [models.UniqueConstraint(fields=["item"], condition=Q(is_cover=True), name="one_cover_per_item")]
    def clean(self):
        if self.is_cover and Photo.objects.exclude(pk=self.pk).filter(item=self.item, is_cover=True).exists():
    ...
    def save(self, *args, **kwargs):
        ...
        with transaction.atomic():
            if self.is_cover:
                Photo.objects.filter(item=self.item, is_cover=True).exclude(pk=self.pk).update(is_cover=False)
            super().save(*args, **kwargs)
    ...
    def delete(self, *args, **kwargs):
        ...
        if thumb_name: self.thumbnail.storage.delete(thumb_name)
        if was_cover:
            replacement = item.photos.first()
            if replacement: replacement.set_as_cover()
        return result
    def __str__(self): return f"{self.item} fotoğrafı"
